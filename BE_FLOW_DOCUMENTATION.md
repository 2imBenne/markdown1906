...
10. Quản Lý Hồ Sơ Bệnh Nhân (Patient Profiles)
11. Bệnh Án & Kết Quả Cận Lâm Sàng (Medical Records)
12. Thông Báo & Nhắc Nhở Uống Thuốc (Notifications & Reminders)
...
### 3.10 Quản Lý Hồ Sơ Bệnh Nhân (Patient Profiles)
* **API Endpoints:** `GET /api/patients`, `POST /api/patients`, `PUT /api/patients`, `DELETE /api/patients`
* **Mô tả:** Người dùng có thể tạo và quản lý nhiều hồ sơ bệnh nhân (cho bản thân hoặc người nhà). Hệ thống sẽ tự động sinh mã bệnh nhân ngẫu nhiên và kiểm tra tính duy nhất của CMND/CCCD và thẻ BHYT để tránh trùng lặp dữ liệu. Khi xóa sẽ thực hiện Soft Delete (`is_deleted=true`).
* **Các file liên quan:** `controllers/patient_controller.go`, `models/patient.go`.

```mermaid
sequenceDiagram
    autonumber
    actor User as Bệnh nhân
    participant API as Patient Controller
    participant DB as MySQL Database
    participant Notify as Notification Module

    User->>API: POST /api/patients (Thông tin hồ sơ)
    API->>DB: Kiểm tra trùng lặp CCCD hoặc BHYT
    alt Bị trùng lặp
        DB-->>API: Conflict Error
        API-->>User: Lỗi "CCCD/BHYT đã tồn tại"
    else Hợp lệ
        API->>API: Sinh Mã Bệnh Nhân ngẫu nhiên (Ví dụ: BN1234567890)
        API->>DB: Lưu Patient Profile mới
        DB-->>API: Lưu thành công
        API->>Notify: Tạo thông báo "Tạo hồ sơ thành công"
        API-->>User: Trả về dữ liệu hồ sơ bệnh nhân
    end
```

---

### 3.11 Bệnh Án & Kết Quả Cận Lâm Sàng (Medical Records)
* **API Endpoints:** `GET /api/lab-results`, `GET /api/prescriptions`
* **Mô tả:** Bệnh nhân truy xuất lịch sử khám bệnh bao gồm các kết quả xét nghiệm/cận lâm sàng (được KTV cập nhật) và các đơn thuốc (được Bác sĩ kê). API tự động gom nhóm dữ liệu dựa trên danh sách hồ sơ (Patient Profiles) thuộc quyền sở hữu của người dùng.
* **Các file liên quan:** `controllers/record_controller.go`, `models/record.go`.

```mermaid
sequenceDiagram
    autonumber
    actor Patient as Bệnh nhân
    participant API as Record Controller
    participant DB as MySQL Database

    Patient->>API: GET /api/prescriptions (Hoặc /api/lab-results)
    API->>DB: Lấy tất cả Hồ sơ bệnh nhân (PatientProfile) của User này
    DB-->>API: Danh sách PatientIDs
    API->>DB: Lấy tất cả Đơn thuốc/Kết quả dựa theo danh sách PatientIDs
    DB-->>API: Kết quả (Kèm theo thông tin Bác sĩ, Thuốc)
    API-->>Patient: Trả về JSON lịch sử khám bệnh
```

---

### 3.12 Thông Báo & Nhắc Nhở Uống Thuốc (Notifications & Reminders)
* **API Endpoints:** `GET /api/notifications`, `POST /api/reminders`, `GET /api/reminders`
* **Mô tả:** 
  - **Notifications:** Quản lý thông báo hệ thống (nhắc lịch, kết quả xét nghiệm, biến động thanh toán).
  - **Reminders:** Người dùng có thể thiết lập lịch nhắc uống thuốc cá nhân. Hệ thống hỗ trợ tạo hàng loạt nhắc nhở trong 1 Request (Bulk Insert) và tính năng bật/tắt nhắc nhở.
* **Các file liên quan:** `controllers/notification_controller.go`, `controllers/reminder_controller.go`, `models/notification.go`.

```mermaid
sequenceDiagram
    autonumber
    actor User as Bệnh nhân
    participant API as Reminder Controller
    participant DB as MySQL Database

    User->>API: POST /api/reminders (Mảng JSON chứa các giờ nhắc uống thuốc)
    API->>API: Giải mã payload dạng Array
    API->>DB: Bắt đầu Transaction (Begin)
    
    loop Lưu từng nhắc nhở
        API->>DB: Lưu vào bảng medication_reminders
    end
    
    DB-->>API: Thành công
    API->>DB: Commit Transaction
    API-->>User: Phản hồi "Lưu nhắc nhở thành công"
```